...
export default function OwnerTurfsPage() {
  ...
  const fetchTurfs = async () => {
    try {
      ...
      const response = await axios.get('https://turf-booking-1-mns7.onrender.com/owner/turfs', {
        headers: {
  ...
  const confirmDelete = async () => {
    ...
    try {
      // Mock delete
      toast.success("Turf deleted successfully! (Mocked)")
  ...
  const handleStatusChange = async (e: React.MouseEvent, id: string, currentStatus: string) => {
    ...
    try {
      // Mock status change
      toast.success(`Turf status updated to ${newStatus} (Mocked)`)
        ...
        >
          <AnimatePresence>
            {turfs.map((turf) => {
              ...
              const imageUrl = turf.images && turf.images.length > 0 ? getImageUrl(turf.images[0]) : null
              ...
              return (
                ...
                >
                  <Card 
                    ...
                    className="bg-card/40 backdrop-blur-md border-white/5 shadow-lg overflow-hidden group hover:border-brand-mint/30 hover:shadow-brand-mint/5 hover:-translate-y-1 transition-all duration-300 cursor-pointer rounded-2xl h-full flex flex-col"
                  >
                    <div className="h-48 bg-muted relative overflow-hidden">
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-black/40 to-black/80" style={{ display: imageUrl ? 'none' : 'flex' }}>
                         <TentTree className="w-12 h-12 text-white/20 mb-2" />
                         <span className="text-[10px] text-white/40 uppercase tracking-widest font-semibold">No Image</span>
                      </div>
                      ...
                      <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <Button 
                          size="icon" 
                          variant="secondary" 
                          className="w-8 h-8 bg-black/60 hover:bg-black text-white backdrop-blur-md border border-white/10"
                    ...
                    <CardContent className="p-5 flex-1 flex flex-col">
                      <div className="flex justify-between items-start mb-1">
                        <h3 className="font-bold text-xl truncate pr-2 text-white group-hover:text-brand-mint transition-colors duration-300">{turf.name || turf.turf_name}</h3>
                      </div>
      ...
      {selectedTurf && (
        ...
        >
          <div className="mt-4 space-y-6">
            {/* Hero Image */}
            <div className="w-full h-48 sm:h-64 rounded-xl overflow-hidden bg-muted relative">
               {selectedTurf.images && selectedTurf.images.length > 0 && getImageUrl(selectedTurf.images[0]) && !imageError ? (
                  <img 
                    src={getImageUrl(selectedTurf.images[0])!} 
                    alt="Turf Hero" 
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center bg-black/40">
                     <TentTree className="w-16 h-16 text-white/20 mb-2" />
                     <span className="text-xs text-white/40 uppercase tracking-widest font-semibold">Image Not Available</span>
                  </div>
               )}
               <div className="absolute top-4 right-4 bg-black/70 backdrop-blur-md text-white px-3 py-1.5 rounded-lg border border-white/10 font-bold">
            ...
            <div>
              ...
              <p className="text-sm text-white/90 bg-white/5 p-3 rounded-xl border border-white/5 leading-relaxed">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedTurf.sports && Array.isArray(selectedTurf.sports) ? selectedTurf.sports.map((sport: any, i: number) => (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedTurf.amenities && Array.isArray(selectedTurf.amenities) ? selectedTurf.amenities.map((amenity: any, i: number) => (
